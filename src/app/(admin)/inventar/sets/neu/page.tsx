...
export default function NewSetPage() {
  ...
  const toggleProduct = (product: any) => {
    setSelected((prev) => {
      ...
      return [...prev, { id: product.id, name: product.name, product_id: product.product_id, quantity: 1 }];
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
      imageUrl = await uploadImage(inserted.id);
      if (imageUrl) {
        await supabase
          .from("product_sets")
    ...
    if (itemsError) {
      toast.error("Fehler beim Hinzufügen der Artikel: " + itemsError.message);
      return;
    }
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h1 className="page-header mb-6">Set erstellen</h1>
